...
// Note that this pool has no minter key of DARKG (rewards).
// Instead, the governance will call DARKG distributeReward method and send reward to this pool at the beginning.
contract DarkgRewardPool is IRewardPool, ReentrancyGuard {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. DARKGs to distribute in the pool.
        uint256 lastRewardTime; // Last time that DARKGs distribution occurred.
        uint256 accDarkgPerShare; // Accumulated DARKGs per share, times 1e18. See below.
    ...
    // The time when DARKG mining starts.
    ...
    constructor(address _darkg, uint256 _poolStartTime) public {
        require(block.timestamp < _poolStartTime, "late");
        if (_darkg != address(0)) darkg = IERC20(_darkg);

        poolStartTime = _poolStartTime;
        poolEndTime = _poolStartTime.add(RUNNING_TIME);
        rewardPerSecond = TOTAL_REWARDS.div(RUNNING_TIME); // 0.41335978835978837 DARKG/sec
    ...
    function add(uint256 _allocPoint, IERC20 _token, uint256 _lastRewardTime) public onlyOperator {
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accDarkgPerShare: 0, isStarted: _isStarted}));
        if (_isStarted) {
            totalAllocPoint_ = totalAllocPoint_.add(_allocPoint);
        }
    }

    // Update the given pool's DARKG allocation point. Can only be called by the owner.
    ...
    // View function to see pending DARKGs on frontend.
    function pendingReward(uint256 _pid, address _user) public override view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accDarkgPerShare = pool.accDarkgPerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _darkgReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint_);
            accDarkgPerShare = accDarkgPerShare.add(_darkgReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accDarkgPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint_ > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _darkgReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint_);
            pool.accDarkgPerShare = pool.accDarkgPerShare.add(_darkgReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) external override nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accDarkgPerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeDarkgTransfer(msg.sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accDarkgPerShare).div(1e18);
    ...
    function _withdraw(address _account, uint256 _pid, uint256 _amount) internal {
        ...
        uint256 _pending = user.amount.mul(pool.accDarkgPerShare).div(1e18).sub(user.rewardDebt);
        if (_pending > 0) {
            safeDarkgTransfer(_account, _pending);
    ...
    // Safe darkg transfer function, just in case if rounding error causes pool to not have enough DARKGs.
    function safeDarkgTransfer(address _to, uint256 _amount) internal {
        uint256 _darkgBal = darkg.balanceOf(address(this));
        if (_darkgBal > 0) {
            if (_amount > _darkgBal) {
                darkg.safeTransfer(_to, _darkgBal);
            } else {
                darkg.safeTransfer(_to, _amount);
            }
        }
    }
